...
\\ pu_qe instead.
{qe_lobatto(qe,x0,n)=local(x,P); if(n==0, n=length(qe[1])+1);
  if(length(qe[1])<n, qe[1]=concat(qe[1],0));
...
{qe_jacobi(m,alf,bet)=local(kap,n); n=floor((m+1)/2);   
...
{vqe_stratify(qe,Z,k)=local(qe1,first,m,n,mu1,mu2);
...
{ab_radau(ab,xn)= local(n,p); n=length(ab[2]);
...
{ab_shift(ab,int1,int2)= local(f,h,b0); if(int2==0, int2=[-1,1]);
   f=(int1[2]-int1[1])/(int2[2]-int2[1]); h=int1[1]-f*int2[1];
...
{ab_kronrod(ab,n)=local(a,b,nmax,s,t,swap,sj,sk,j,k,l,m,k0,k1);  
...
{ab_wmul1(ab,z)= local(n,a,b,r,A,B,trunc,mu); a=ab[1]; b=drop(ab[2],1); 
...
{ab_wdiv1(ab0,mu0,z)= local(n,r,nu,ab,aug); n=length(ab0[2]); 
  aug=length(ab0[1])==n;
...
{qe_ab(ab)=local(a,b,n); n=length(ab[1]);
  for(i=2,n,
...
{dw_qe(qe)=local(d,w,x,pu,nu,n); d=gaps(qe); n=length(d); dw=matrix(n,2); x=0;
  for(j=1,n, x=x+d[j]; dw[j,1]=d[j]; dw[j,2]=weight(qe,x));
  [0,dw]} 
addhelp(dw_qe, "dw_qe(qe)");

{qe_dw(dw)=local(d,w,u,v,n,mu0); dw[2][1,2]=dw[2][1,2]+dw[1]; dw=dw[2]; 
...
{dw_xw(xw)=local(n); n=matsize(xw)[1];
  forstep(k=n,2,-1, xw[k,1]=xw[k,1]-xw[k-1,1]); [0,xw]}
addhelp(dw_xw, "dw_xw(xw)");

{xw_dw(dw)=local(n); dw[2][1,1]=dw[2][1,1]+dw[1]; dw=dw[2]; n=matsize(dw)[1];
  for(k=2,n, dw[k,1]=dw[k,1]+dw[k-1,1]); dw}
addhelp(xw_dw, "xw_dw(dw)");

{xw_ab(ab)=local(x,xw,n); x=qlpoles(ab); n=length(x); xw=matrix(n,2);
  DEBUG=1;
...
{ab_dw(dw)=local(h); h=dw[2][1,1]; dw[2][1,1]=0; 
...
{stqd(qe,sig)=local(q,e,n,t,u,f); q=qe[1]; e=qe[2]; n=length(q); t=-sig; 
 for(i=1,n-1, u=q[i]; q[i]=q[i]+t; f=e[i]/q[i]; e[i]=u*f; t=t*f-sig);
 q[n]=q[n]+t; [q,e,qe[3]]}
addhelp(stqd, "stqd(qe,{sig=0}): stationary qd algorithm");

{prqd(qe,sig)=local(q,e,n,c,d,f,t); q=qe[1]; e=qe[2]; n=length(q); t=-sig;
...
{prql(T,sig,CS)=local(n,alf,gam,oldgam,oldC,BB,P,R,C,S,CSgiven);
...
{prqr(T,sig,CS)=local(n,alf,gam,oldgam,oldC,BB,P,R,C,S,CSgiven);
...
{r_qe(qe,x)=local(f,n,c); q=qe[1]; e=qe[2]; n=length(q); c=qe[3]; r=0; 
  forstep(j=n,1,-1, r=if(j>1,e[j-1],c)/(x-q[j]/(1-r))); r}

{r_ab(ab,x)=local(r,n); a=ab[1]; b=ab[2]; n=length(a);
  r=0; forstep(j=n,1,-1, r=b[j]/(x-a[j]-r)); r}

{r_xw(xw,t)=sum(j=1,matsize(xw)[1], xw[j,2]/(t-xw[j,1]))}

{stieltjes_function(u,x)=local(typ); typ=Type(u);
...
{o_pol(u,x)=local(typ); typ=Type(u);
...
{mu_qe(qe,Z,flag)=local(Q,E,N,q,e,n,mu,ro,d,m,s);
...
{qe_mu(nu,Z,n)=local(q,e,Q,E,m,ro,nu1); Q=Z[1]; E=Z[2];
...
{xw_shift(xw,int1,int2)= local(f,h); if(int2==0, int2=[-1,1]);
   f=(int1[2]-int1[1])/(int2[2]-int2[1]); h=int1[1]-f*int2[1];
...
{o_mulx(c,ab)=local(n); n=length(c); check_length(ab,n,"o_mulx"); 
  vector(n+1,l, 
    if(l>1,c[l-1]) + if(l<n,ab[2][l]*c[l+1]) + if(l<=n,ab[1][l]*c[l]))}
{o_ser(c,ab,x)=local(n,f,f0,f1);  n=length(c); check_length(ab,n-1,"o_ser");
  f1=0; f=c[n]; forstep(k=n-1,1,-1, 
    f0=f1; f1=f; f=(x-ab[1][k])*f1-if(k<n-1,ab[2][k]*f0)+c[k]); 
  f}
{o_dot(c1,c2,nu)=local(n); n=min(length(c1),length(c2)); 
  check_length(ab,n,"o_dot"); sum(i=1,n, c1[i]*c2[i]*nu[i])}
 
\\ Recursion coefficients for refinable functions
{ab_refinable(n,g)= local(ab,ckk,p0,p1,p2,nu,M,N,symm); M=length(g); 
...
\\ Various
{exactness(Z1,Z2,eps)=local(n,N,d,j);
...
integrate(fct,xw)=local(s); for(j=1,matsize(xw)[1], x=xw[j,1]; s=s+xw[j,2]*eval(fct)); s

{stratified_integrate(vxw)=local(m,y); m=length(vxw); y=vector(m);
...
{findpolynomial(u)=local(pk); for(k=1,length(SHORT_POLYNOMIALS), 
...
{AB(u,deg,p1,p2)= local(typ,m,n); typ=Type(u); 
...
{QE(u,deg,p1,p2)= local(typ,m,n); typ=Type(u);
  if (typ=="t_INT", deg=u; u="Leg"; typ="t_STR");
  if (typ=="t_STR", typ=findpolynomial(u); 
    if (typ=="Che", p1=-1/2; p2=p1; typ="Jac");
    if (typ=="Leg", p1=0; p2=0; typ="Jac");
...
{XW(u,npts,p1,p2)= local(typ,m,n,deg); typ=Type(u); 
...
{DW(u,npts,p1,p2)= local(typ,m,n,deg); typ=Type(u);
  if (typ=="t_INT", npts=u; u="Leg"; typ="t_STR");
  deg=2*npts-1;
  if (typ=="t_STR", typ=findpolynomial(u); 
    if (typ=="Che", p1=-1/2; p2=p1; typ="Jac");
...
{Take(R,dim,typ)= local(n); if(dim==0, return(R));
  if (typ==0, typ=Type(R));
...
{Type(u)=local(typ,typ2,len,rtyp); rtyp=0; typ=type(u); len=length(u);
...
{Check(u,typ,force)= local(OK); if (force+DEBUG==0, return);
  OK=1;
